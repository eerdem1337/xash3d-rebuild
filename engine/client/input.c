...
void IN_EngineAppendMove( float frametime, usercmd_t *cmd, qboolean active )
{
	...
	if( active )
	{
		...
		if( k_silentaim.value && ( cmd->buttons & IN_ATTACK ))
		{
			...
			VectorAdd( cl.simorg, cl.viewheight, eyePos );
			...
			for( i = 1; i <= cl.maxclients; i++ )
			{
				...
				VectorSubtract( ent->origin, eyePos, delta );
			...
			if( target )
			{
				vec3_t targetAngles, view;
				VectorSubtract( target->origin, eyePos, view );
				VectorAngles( view, targetAngles );
				VectorCopy( targetAngles, cmd->viewangles );
			}
		}
