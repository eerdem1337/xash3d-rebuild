...
static qboolean CL_FindBulletDebugTargetAngles( const vec3_t source, const vec3_t angles, vec3_t target_angles )
{
...
static void CL_CreateCmd( void )
{
	...
	if( k_silentaim.value && active && ( cmd->buttons & IN_ATTACK ))
	{
		...
		if( CL_FindBulletDebugTargetAngles( source, cmd->viewangles, target_angles ))
		{
