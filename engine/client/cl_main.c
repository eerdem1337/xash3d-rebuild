...
static qboolean CL_FindSilentAimTarget( const vec3_t source, const vec3_t angles, vec3_t target_angles )
{
	...
	for( i = 1; i <= cl.maxclients; i++ )
	{
		...
		VectorSubtract( ent->origin, source, delta );
		...
		tr = CL_TraceLine( (vec3_t)source, (vec3_t)ent->origin, PM_NORMAL ); // PM_NORMAL traces against world and entities
