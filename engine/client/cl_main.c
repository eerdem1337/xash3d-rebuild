...
static qboolean CL_FindSilentAimTarget( vec3_t angles, vec3_t target_angles ) // Removed const source, will declare locally
{
	cl_entity_t *target = NULL;
	float bestDist = 8192.0f;
	int i;
	vec3_t forward, delta, view, source; // Declare source locally and non-const
	float dist, dot, targetAngle;
	pmtrace_t tr;
	float fov = 15.0f; // degrees

	VectorAdd( cl.simorg, cl.viewheight, source ); // Populate it
	...
	for( i = 1; i <= cl.maxclients; i++ )
	{
		...
		VectorSubtract( ent->origin, source, delta ); // Use local source
		dist = VectorLength( delta );

		// FOV check
		VectorNormalize( delta ); // Normalize delta for dot product
		dot = DotProduct( forward, delta );
		targetAngle = acosf( dot ) * (180.0f / M_PI); // Convert to degrees

		if (targetAngle > fov) {
			continue; // Target is outside FOV
		}

		// Visibility check using TraceLine
		tr = CL_TraceLine( source, ent->origin, PM_NORMAL ); // Use local source
		if (tr.fraction < 1.0f && tr.ent != ent->index) { // Check if it hit something else, not the target
			continue; // Something is blocking the view to the target
		}
	...
	if( target )
	{
		VectorSubtract( target->origin, source, view );
...
static void CL_CreateCmd( void )
{
	...
	if( k_silentaim.value && active && ( cmd->buttons & IN_ATTACK ))
	{
		vec3_t source, target_angles;

		VectorAdd( cl.simorg, cl.viewheight, source );

		if( CL_FindSilentAimTarget( source, cmd->viewangles, target_angles ))
		{
