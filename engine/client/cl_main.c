...
CVAR_DEFINE_AUTO( k_esp, "0", FCVAR_ARCHIVE, "show player esp" );
CVAR_DEFINE_AUTO( k_silentaim, "0", FCVAR_ARCHIVE, "enable silent aimbot" );
CVAR_DEFINE_AUTO( k_aimbot, "0", FCVAR_ARCHIVE, "enable bone aimbot" );
...
#include "pm_defs.h" // For pmtrace_t and PM_ flags
#include <math.h> // For acosf and M_PI

#ifndef M_PI
#define M_PI 3.14159265358979323846
#endif

static qboolean CL_GetBoneTarget( cl_entity_t *ent, vec3_t target_pos )
{
	studiohdr_t *hdr = (studiohdr_t *)Mod_StudioExtradata( ent->model );
	if( !hdr || hdr->hitboxindex == 0 ) return false;

	mstudiobbox_t *pbbox = (mstudiobbox_t *)((byte *)hdr + hdr->hitboxindex);

	vec3_t bone_origin;
	VectorAdd( pbbox[0].bbmin, pbbox[0].bbmax, bone_origin );
	VectorScale( bone_origin, 0.5f, bone_origin );
	
	VectorAdd( ent->origin, bone_origin, target_pos );
	return true;
}

static qboolean CL_FindAimbotTarget( const vec3_t source, const vec3_t angles, vec3_t target_angles )
{
	cl_entity_t *target = NULL;
	vec3_t target_pos, bestTargetPos, delta, view, forward;
	float bestDist = 8192.0f;
	float dist, dot, targetAngle;
	pmtrace_t tr;
	int i;
	float fov = 30.0f; 

	AngleVectors( angles, forward, NULL, NULL );
	VectorClear( bestTargetPos );

	for( i = 1; i <= cl.maxclients; i++ )
	{
		cl_entity_t *ent = CL_GetEntityByIndex( i );

		if( !ent || !ent->player || ent->index == ( cl.playernum + 1 ) || (ent->curstate.effects & EF_NODRAW))
			continue;

		if( !CL_GetBoneTarget( ent, target_pos ))
			VectorCopy( ent->origin, target_pos );

		VectorSubtract( target_pos, source, delta );
		dist = VectorLength( delta );

		VectorNormalize( delta ); 
		dot = DotProduct( forward, delta );
		targetAngle = acosf( dot ) * (180.0f / M_PI);

		if( targetAngle > fov ) continue;

		tr = CL_TraceLine( (vec3_t)source, (vec3_t)target_pos, PM_NORMAL );
		if( tr.fraction < 1.0f && tr.ent != ent->index ) continue;

		if( dist < bestDist )
		{
			bestDist = dist;
			target = ent;
			VectorCopy( target_pos, bestTargetPos );
		}
	}

	if( target )
	{
		VectorSubtract( bestTargetPos, source, view );
		VectorAngles( view, target_angles );
		return true;
	}
	return false;
}
...
static void CL_CreateCmd( void )
{
	...
	if( (k_silentaim.value || k_aimbot.value) && active && ( cmd->buttons & IN_ATTACK ))
	{
		vec3_t target_angles, source;
		VectorAdd( cl.simorg, cl.viewheight, source );

		if( CL_FindAimbotTarget( source, cmd->viewangles, target_angles ))
		{
...
static void CL_InitLocal( void )
{
	...
	Cvar_RegisterVariable( &k_esp );
	Cvar_RegisterVariable( &k_silentaim );
	Cvar_RegisterVariable( &k_aimbot );
