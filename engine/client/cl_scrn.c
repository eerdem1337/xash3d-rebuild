	...
	for( i = 0; i < clgame.maxEntities; i++ )
	{
		...
		if( k_esp.value && ent->player )
		{
			...
			if( !ref.dllFuncs.WorldToScreen( mins, screenMin ) && !ref.dllFuncs.WorldToScreen( maxs, screenMax ))
			{
				...
				ref.dllFuncs.FillRGBA( kRenderTransTexture, (float)x, (float)y, (float)w, 1, 255, 0, 0, 255 );
				ref.dllFuncs.FillRGBA( kRenderTransTexture, (float)x, (float)y + h, (float)w, 1, 255, 0, 0, 255 );
				ref.dllFuncs.FillRGBA( kRenderTransTexture, (float)x, (float)y, 1, (float)h, 255, 0, 0, 255 );
				ref.dllFuncs.FillRGBA( kRenderTransTexture, (float)x + w, (float)y, 1, (float)h, 255, 0, 0, 255 );

				Con_DrawString( x, y - 10, cl.players[ent->index - 1].name, color );
			}
