	...
	for( i = 0; i < clgame.maxEntities; i++ )
	{
		...
		if( k_esp.value && ent->player )
		{
			...
			if( !ref.dllFuncs.WorldToScreen( mins, screenMin ) && !ref.dllFuncs.WorldToScreen( maxs, screenMax ))
			{
				...
				int x = (int)Q_min(screenMin[0], screenMax[0]);
				int y = (int)Q_min(screenMin[1], screenMax[1]);
				int w = (int)Q_abs(screenMax[0] - screenMin[0]);
				int h = (int)Q_abs(screenMax[1] - screenMin[1]);
