...
void SCR_DrawEnts( void )
{
	...
	if( cls.state != ca_active || ( !cl_showents.value && !k_esp.value ))
		return;
	...
	for( i = 0; i < clgame.maxEntities; i++ )
	{
		...
		if( k_esp.value && ent->player )
		{
			// Con_Printf("ESP: Found player %d, origin: %f %f %f\n", ent->index, ent->origin[0], ent->origin[1], ent->origin[2]);
			vec3_t mins, maxs, origin;
			vec3_t screenMin, screenMax;
			int x, y, w, h;

			VectorCopy( ent->origin, origin );
			VectorAdd( origin, ent->curstate.mins, mins );
			VectorAdd( origin, ent->curstate.maxs, maxs );

			if( !ref.dllFuncs.WorldToScreen( mins, screenMin ) && !ref.dllFuncs.WorldToScreen( maxs, screenMax ))
			{
				screenMin[0] = 0.5f * screenMin[0] * refState.width + 0.5f * refState.width;
				screenMin[1] = -0.5f * screenMin[1] * refState.height + 0.5f * refState.height;
				screenMax[0] = 0.5f * screenMax[0] * refState.width + 0.5f * refState.width;
				screenMax[1] = -0.5f * screenMax[1] * refState.height + 0.5f * refState.height;

				x = (int)Q_min(screenMin[0], screenMax[0]);
				y = (int)Q_min(screenMin[1], screenMax[1]);
				w = (int)fabsf(screenMax[0] - screenMin[0]);
				h = (int)fabsf(screenMax[1] - screenMin[1]);

				int cornerLength; // Declare here
				cornerLength = Q_min(w, h) / 4; // Adjust corner length as desired
				// Draw corner ESP

				// Top-left corner
				ref.dllFuncs.FillRGBA( kRenderTransTexture, (float)x, (float)y, cornerLength, 1, 255, 0, 0, 255 );
				ref.dllFuncs.FillRGBA( kRenderTransTexture, (float)x, (float)y, 1, cornerLength, 255, 0, 0, 255 );
				// Top-right corner
				ref.dllFuncs.FillRGBA( kRenderTransTexture, (float)x + w - cornerLength, (float)y, cornerLength, 1, 255, 0, 0, 255 );
				ref.dllFuncs.FillRGBA( kRenderTransTexture, (float)x + w, (float)y, 1, cornerLength, 255, 0, 0, 255 );
				// Bottom-left corner
				ref.dllFuncs.FillRGBA( kRenderTransTexture, (float)x, (float)y + h, cornerLength, 1, 255, 0, 0, 255 );
				ref.dllFuncs.FillRGBA( kRenderTransTexture, (float)x, (float)y + h - cornerLength, 1, cornerLength, 255, 0, 0, 255 );
				// Bottom-right corner
				ref.dllFuncs.FillRGBA( kRenderTransTexture, (float)x + w - cornerLength, (float)y + h, cornerLength, 1, 255, 0, 0, 255 );
				ref.dllFuncs.FillRGBA( kRenderTransTexture, (float)x + w, (float)y + h - cornerLength, 1, cornerLength, 255, 0, 0, 255 );

				// Draw player name above the box, centered
				const char *playerName = cl.players[ent->index - 1].name;
				int nameWidth, nameHeight;
				Con_DrawStringLen( playerName, &nameWidth, &nameHeight );
				Con_DrawString( x + (w / 2) - (nameWidth / 2), y - nameHeight - 2, playerName, color ); // -2 for slight offset

				// Draw weapon name below the box, centered
				if( ent->curstate.weaponmodel > 0 )
				{
					model_t *weaponModel = CL_ModelHandle( ent->curstate.weaponmodel );
					if( weaponModel && weaponModel->name[0] != '\0' ) // Check if model name is valid
					{
						const char *weaponName = weaponModel->name;
						int weaponNameWidth, weaponNameHeight;
						Con_DrawStringLen( weaponName, &weaponNameWidth, &weaponNameHeight );
						Con_DrawString( x + (w / 2) - (weaponNameWidth / 2), y + h + 2, weaponName, color ); // +2 for slight offset
					}
				}
			}
			continue; // Skip the rest of the debug info for this entity
		}

		if( !cl_showents.value ) continue;
